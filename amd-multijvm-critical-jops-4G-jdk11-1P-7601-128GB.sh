...
sudo sh -c "echo 1 > /proc/sys/kernel/sched_compat_yield"
...
sudo sh -c "echo 0 | tee /proc/sys/kernel/numa_balancing"
}
...
GROUP_COUNT=$(lscpu|grep 'NUMA node(s)'|sed -e 's/.* //') #2
...
SPEC_OPTS_C=" -Dspecjbb.group.count=$GROUP_COUNT -Dspecjbb.txi.pergroup.count=$TI_JVM_COUNT -Dspecjbb.heartbeat.period=2000 -Dspecjbb.heartbeat.threshold=600000 -Dspecjbb.controller.handshake.timeout=800000 -Dspecjbb.forkjoin.workers.Tier1=64 -Dspecjbb.forkjoin.workers.Tier2=1 -Dspecjbb.forkjoin.workers.Tier3=24 -Dspecjbb.comm.connect.timeouts.connect=700000 -Dspecjbb.comm.connect.timeouts.read=700000 -Dspecjbb.comm.connect.timeouts.write=700000 -Dspecjbb.comm.connect.selector.runner.count=0 -Dspecjbb.mapreducer.pool.size=4"
...
JAVA_OPTS_BE="-showversion -server -XX:AllocatePrefetchInstr=2 -XX:LargePageSizeInBytes=2m -XX:-UsePerfData -XX:+AggressiveOpts -XX:-UseAdaptiveSizePolicy -XX:+AlwaysPreTouch -XX:-UseBiasedLocking -XX:+UseLargePages -XX:+UseParallelOldGC -Xms56g -Xmx56g -Xmn54g -XX:SurvivorRatio=52 -XX:TargetSurvivorRatio=98 -XX:ParallelGCThreads=32 -XX:MaxTenuringThreshold=15 -Xnoclassgc -XX:InlineSmallCode=10k -XX:MaxGCPauseMillis=300 -XX:ThreadStackSize=1m -XX:+UseCompressedOops -XX:ObjectAlignmentInBytes=16 -XX:+UseNUMA -XX:+PrintFlagsFinal "
